...
ShaderInstance::ShaderInstance(EShaderType shaderType, const char* shaderSource)
{
    ...
    {
        std::cerr << "ERROR::SHADER::INSTANCE GLShaderType for ShaderType " << shaderType
    ...
    if (!success)
    {
        ...
        std::cerr << "ERROR::SHADER::INSTANCE::" << shaderType << "::COMPILATION_FAILED\n"
                  << infoLog << std::endl;
        throw -1;
    }
}
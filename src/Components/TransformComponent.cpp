...
glm::mat4 UTransformComponent::ConstructTransformMatrix() const {
...

    {
        TransformMatrix = glm::translate(
            TransformMatrix, glm::vec3(Position));

        // TODO: find a way to do it in single operation using quat
        // pitch
        TransformMatrix = glm::rotate(
            TransformMatrix, glm::radians(Rotation.z),
            glm::vec3(1.0f, 0.f, 0.0f));

        // yaw
        TransformMatrix = glm::rotate(
            TransformMatrix, glm::radians(Rotation.x),
            glm::vec3(0.0f, 1.f, 0.0f));

        // roll
        TransformMatrix = glm::rotate(
            TransformMatrix, glm::radians(Rotation.y),
            glm::vec3(0.0f, 0.f, 1.0f));

        TransformMatrix =
            glm::scale(TransformMatrix, Scale);
    };
